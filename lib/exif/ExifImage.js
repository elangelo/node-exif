...
function ExifImage (options, callback) {
	...
	this.exifData = {
		...
		thumbnailinfo : {},							// Information about the thumbnail
  ...
  {
		this.loadImage(options.image, function (error, exifData) 
    {
			if (error) 
      {
         callback(error);
...
ExifImage.prototype.loadImage = function (image, callback) 
{
    ...
    {
		    this._reader = binaryreader.open(image)
        .on ('error', function (error) { callback(error); })
        .on ('close', function() { callback(self._error, self._error ? null : self.exifData); })
        .read(2, this.checkImageType)
        .read(2, this.checkExif)
        .read(2, function(bytesRead, bytes) 
        {
           var exifheaderlength = bytes.getShort(0, true) - 2;
           console.log('exifheaderlength :' + exifheaderlength);
           self._reader.read(exifheaderlength, function(bytesRead2, bytes2)
           {
             try
             {
                self.ProcessExifData(bytes2, callback);
             }
             catch (error) 
             {
               self._error = error;
...
ExifImage.prototype.checkImageType = function(bytesRead, bytes) 
{
    var self = this;
    ...
    {
        this._error = new Error('no jpeg found, will halt');
        this._reader.cancel();
    }
};

ExifImage.prototype.checkExif = function(bytesRead, bytes) 
{
    var self = this;
    if (bytes[0] != 0xFF && bytes[1] != 0xE1) 
    {
      this._error = new Error('invalid EXIF marker found');
      this._reader.cancel();
    }
};

ExifImage.prototype.ProcessExifData = function(bytes)
{
    var self = this
    var byteOffset = 0
    var positionInFile = 6;

    if (bytes.toString('utf8', byteOffset, 6) != 'Exif\0\0')
    {
       callback(new Error('invalid EXIF marker found'));
    }

    byteOffset = byteOffset + 6;

    if (bytes.getShort(byteOffset) == 0x4949)
    {
    ...
    if (bytes.getShort(byteOffset, this.isBigEndian) != 0x002A) 
    {
		var expected = (this.isBigEndian) ? '0x002A' : '0x2A00';
        callback(new Error('Invalid TIFF data! Expected ' + expected));
	}

    byteOffset = byteOffset + 2
    
    ifdOffset = byteOffset + bytes.getLong(byteOffset, this.isBigEndian) + 2 - positionInFile; // don't know where the two comes from but i need it.
    
    numberOfEntries = bytes.getShort(ifdOffset, this.isBigEndian);

    /********************************* IFD0 **********************************/
    // Each IFD entry consists of 12 bytes which we loop through and extract the data from
    for (var i =0 ; i< numberOfEntries; i++) 
    {
        var exifEntry = self.extractExifEntry(bytes, (ifdOffset + 2 + (i * 12)), byteOffset + 2 - positionInFile, this.isBigEndian, ExifImage.TAGS.exif);
	    if (exifEntry && exifEntry.tagName !== null)
        {
             this.exifData.image[exifEntry.tagName] = exifEntry.value;
        }
    }
	...
	var nextIfdOffset = bytes.getLong(ifdOffset + 2 + (numberOfEntries * 12), this.isBigEndian)
	if (nextIfdOffset != 0x00000000) {

		ifdOffset = byteOffset + nextIfdOffset + 2 - positionInFile; // don't know where the two comes from but i need it.
		numberOfEntries = bytes.getShort(ifdOffset, this.isBigEndian);
		...
		for (var i = 0; i < numberOfEntries; i++) {
			...
			if (exifEntry && exifEntry.tagName !== null) 
            { 
                this.exifData.thumbnailinfo[exifEntry.tagName] = exifEntry.value; 
            }
		}
	}
	...
	if (typeof this.exifData.image[ExifImage.TAGS.exif[0x8769]] != "undefined") {
		...
		ifdOffset = byteOffset + this.exifData.image[ExifImage.TAGS.exif[0x8769]] + 2 - positionInFile;
		numberOfEntries = bytes.getShort(ifdOffset, this.isBigEndian);
		...
		for (var i = 0; i < numberOfEntries; i++) {
			...
			if (exifEntry && exifEntry.tagName !== null) 
            {
                this.exifData.exif[exifEntry.tagName] = exifEntry.value;
            }
		}
	}
    /******************************** GPS IFD ********************************/
	...
	if (typeof this.exifData.image[ExifImage.TAGS.exif[0x8825]] != "undefined") {
		...
		ifdOffset = byteOffset + this.exifData.image[ExifImage.TAGS.exif[0x8825]] + 2 - positionInFile;
		numberOfEntries = bytes.getShort(ifdOffset, this.isBigEndian);
		
		// Each IFD entry consists of 12 bytes which we loop through and extract
		// the data from
		for (var i = 0; i < numberOfEntries; i++) {
			var exifEntry = self.extractExifEntry(bytes, (ifdOffset + 2 + (i * 12)), byteOffset + 2 - positionInFile, this.isBigEndian, ExifImage.TAGS.gps);
	...
	if (typeof this.exifData.exif[ExifImage.TAGS.exif[0xA005]] != "undefined") {
		...
		ifdOffset = byteOffset + this.exifData.exif[ExifImage.TAGS.exif[0xA005]] + 2 - positionInFile;
		numberOfEntries = bytes.getShort(ifdOffset, this.isBigEndian);
		...
		for (var i = 0; i < numberOfEntries; i++) {
			...
			if (exifEntry && exifEntry.tagName !== null) 
            {
                this.exifData.interoperability[exifEntry.tagName] = exifEntry.value;
            }
		}
	...
	if (typeof this.exifData.exif[ExifImage.TAGS.exif[0x927C]] != "undefined") {
		...
		if (self.exifData.exif[ExifImage.TAGS.exif[0x927C]].getString(0, 7) === "OLYMP\x00\x01" || self.exifData.exif[ExifImage.TAGS.exif[0x927C]].getString(0, 7) === "OLYMP\x00\x02") {
			self.extractMakernotes = require('./makernotes/olympus').extractMakernotes;
		} else if (self.exifData.exif[ExifImage.TAGS.exif[0x927C]].getString(0, 7) === "AGFA \x00\x01") {
			self.extractMakernotes = require('./makernotes/agfa').extractMakernotes;
		} else if (self.exifData.exif[ExifImage.TAGS.exif[0x927C]].getString(0, 8) === "EPSON\x00\x01\x00") {
			self.extractMakernotes = require('./makernotes/epson').extractMakernotes;
		} else if (self.exifData.exif[ExifImage.TAGS.exif[0x927C]].getString(0, 8) === "FUJIFILM") {
			self.extractMakernotes = require('./makernotes/fujifilm').extractMakernotes;
		} else {
			...
			self.exifData.makernote['error'] = 'Unable to extract Makernote information as it is in an unsupported or unrecognized format.';
		}
		if (self.extractMakernotes) 
        {
		    self.exifData.makernote = self.extractMakernotes(bytes, self.makernoteOffset, byteOffset + 2 - positionInFile);
        }
	}

    if (this.exifData.thumbnailinfo && this.exifData.thumbnailinfo[ExifImage.TAGS.exif[0x0201]] && this.exifData.thumbnailinfo[ExifImage.TAGS.exif[0x0202]])
    {
        var thumbposition = this.exifData.thumbnailinfo[ExifImage.TAGS.exif[0x0201]] +byteOffset +     2 - positionInFile;
        var thumblength = this.exifData.thumbnailinfo[ExifImage.TAGS.exif[0x0202]];
        var endposition = thumbposition + thumblength;

        self.exifData.thumbnail = bytes.slice(thumbposition, endposition);
    }
}
