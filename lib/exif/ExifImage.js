	...
	if (image.constructor.name === 'String')
    {
		var reader = binaryreader.open(image)
         ...
         .read(2, function(bytesRead, bytes) 
         {
            ...
            {
                 console.log(bytes2.toString('utf-8', 0, 6));
...
ExifImage.prototype.ProcessExifData = function(bytes)
{
    ...
    if (bytes.toString('utf8', 0, 6) != 'Exif\0\0')
    {
       console.log('no clue');
    }
    console.log(bytes.getShort(6));
    if (bytes.getShort(6) == 0x4949)
    {
    ...
    if (bytes.getShort(8, this.isBigEndian) != 0x002A) {
