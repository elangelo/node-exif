...
function ExifImage (options, callback) {
    ...
    {
		this.loadImage(options.image, function (error) 
        {
			if (error) 
				callback(error);
			else
            {
				callback(false, this.imageType);
            }
		});
	}
}
...
ExifImage.prototype.loadImage = function (image, callback) 
{
	...
	if (image.constructor.name === 'String')
    {
        ...
        .on ('close', function() { console.log('closing'); })
        ...
        .read(2, function(bytesRead, bytes) 
        {
           ...
           console.log('exif header length: ' + exifheaderlength);
           
           reader.read(exifheaderlength, function(bytesRead2, bytes2, callback) 
           {
...
ExifImage.prototype.checkImageType = function(bytesRead, bytes, callback) 
{
    ...
    {
        console.log('found a jpeg');
        self.imageType = 'JPEG';
        callback();
    }
    else 
    {
        console.error('no jpeg found, will halt');
...
{
    var self = this;
    console.log('from within checkExif: ' + self.imageType);
    if (bytes[0] != 0xFF && bytes[1] != 0xE1) 
    {
        console.error ('invalid EXIF marker found');
...
ExifImage.prototype.ProcessExifData = function(bytes, callback)
{
	console.log(bytes);
    var self = this
    var byteOffset = 0
    ...
    console.log ('Is Big Endian : ' + self.isBigEndian);
    ...
    {
        var exifEntry = self.extractExifEntry(bytes, (ifdOffset + 2 + (i * 12)), byteOffset, this.isBigEndian, ExifImage.TAGS.exif);
	...
	if (nextIfdOffset != 0x00000000) {
		...
		numberOfEntries = bytes.getShort(ifdOffset, this.isBigEndian);
		console.log('number of entries in ifd1: ' + numberOfEntries);
		...
		for (var i = 0; i < numberOfEntries; i++) {
			...
			if (exifEntry && exifEntry.tagName !== null) 
            { 
                this.exifData.thumbnail[exifEntry.tagName] = exifEntry.value; 
            }
		}
	}
	...
	if (typeof this.exifData.image[ExifImage.TAGS.exif[0x8825]] != "undefined") {
		...
		// the data from
		for (var i = 0; i < numberOfEntries; i++) {
			var exifEntry = self.extractExifEntry(bytes, (ifdOffset + 2 + (i * 12)), byteOffset, this.isBigEndian, ExifImage.TAGS.gps);
	...
	if (typeof this.exifData.exif[ExifImage.TAGS.exif[0x927C]] != "undefined") {
		...
		if (this.extractMakerNotes) 
        {
		    this.exifData.makernote = this.extractMakernotes(bytes, self.makernoteOffset, byteOffset);
        }
	}
...
ExifImage.prototype.processImage = function (reader, callback) 
{
	var self = this;
	var offset = 0;

    //////////while (offset < reader.size()){
    //////////    reader.read(1, function (bytesRead, buffer, callback) {
    //////////           if (buffer[offset++] != 0xFF) {
    //////////              console.error('Invalid marker found at offset '+(--offset)+'. Expected 0xFF but found 0x'+data[offset].toString(16).toUpperCase()+".");
    //////////              reader.cancel();
    //////////              }

    //////////           if (buffer [offset++] == 0xE1) {
    //////////              var exifData = self.extractExifData(reader, offset + 2, buffer.getShort(offset, true) - 2);
	//////////              callback(false, exifData);
    //////////              }
    //////////           else {
    //////////               offset += buffer.getShort(offset, true);
    //////////               }
    //////////        });
