...
ExifImage.prototype.loadImage = function (image, callback) 
{
    ...
    {
		var reader = binaryreader.open(image)
        .on ('error', function (error) { 'error: ' + console.error(error); })
        .on ('close', function() { callback(false, self.exifData); })
        .read(2, this.checkImageType)
        .read(2, this.checkExif)
        .read(2, function(bytesRead, bytes, callback) 
        {
           var exifheaderlength = bytes.getShort(0, true) - 2;
           reader.read(exifheaderlength, function(bytesRead2, bytes2)
           {
...
ExifImage.prototype.checkImageType = function(bytesRead, bytes, callback) 
{
    var self = this;
    ...
    {
        callback(new Error('no jpeg found, will halt'));
    }
};

ExifImage.prototype.checkExif = function(bytesRead, bytes, callback) 
{
    var self = this;
